// lib/hooks/useSubscription.js - ENHANCED REALTIME VERSION
...
export function useSubscription(majstorId) {
      ...
      const requestPromise = (async () => {
        try {
          ...
          if (subscriptions && subscriptions.length > 0) {
            ...
            // 🔥 IMPROVED: Check cancel_at_period_end flag
            ...
            if (subscription.status === 'trial') {
              ...
              // 🔥 CANCELLED status može značiti "active but will cancel"
              if (periodEnd > now) {
                isActive = true
              } else {
                isExpired = true
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (oldCancelFlag === false && newCancelFlag === true) {
            console.log('🚫 CANCEL CONFIRMED via Realtime!')
          }

          // 🔥 DETECT REACTIVATE CONFIRMATION
          if (oldCancelFlag === true && newCancelFlag === false) {
          ...
          // 🔥 NEW: Dispatch cancel/reactivate events
          if (oldCancelFlag === false && newCancelFlag === true) {
  ...
  const isActive = useCallback(() => {
    ...
    // 🔥 IMPROVED: cancelled status može biti active ako period nije završen
  ...
  return {
    ...
    // 🔥 IMPROVED: isCancelled koristi cancel_at_period_end flag
