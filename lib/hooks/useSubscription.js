    ...
    const requestPromise = (async () => {
      try {
        ...
        if (!latestSubscription) {
          ...
          const now = new Date()
          let subscription = { ...latestSubscription } // Shallow copy
          const periodEnd = new Date(subscription.current_period_end)
          
          let needsDbUpdate = false
          let dbUpdateFields = {}

          // 🔥 NORMALIZUJ STATUS: trialing → trial
          if (subscription.status === 'trialing') {
            subscription.status = 'trial'
            console.log('🔄 Normalized status: trialing → trial')
          }

          // 🔥 DETEKCIJA cancelled_at
          if (subscription.cancelled_at && !subscription.status.includes('cancel')) {
            const cancelledDate = new Date(subscription.cancelled_at)
            
            if (cancelledDate <= now) {
              console.log('🔥 DETECTED cancelled_at:', subscription.cancelled_at)
              console.log('Old status:', subscription.status)
              
              subscription.status = 'cancelled'
              needsDbUpdate = true
              dbUpdateFields.status = 'cancelled'
              
              console.log('✅ Will update DB to cancelled status')
            }
          }

          // 🔥 AŽURIRAJ BAZU AKO JE POTREBNO
          if (needsDbUpdate) {
            console.log('💾 Updating database with new status:', dbUpdateFields)
          ...
          // Proveri da li je aktivna pretplata
          let isActive = false
          let isExpired = false
