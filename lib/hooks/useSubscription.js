...
export function useSubscription(majstorId) {
  ...
  const hasFeatureAccess = useCallback((featureKey) => {
    ...
    // Grace period: freemium users with active grace_until get PRO access (not PRO+)
    if (plan.name === 'freemium' && graceUntil && new Date(graceUntil) > new Date()) {
      return featureKey !== 'team'
    }
    ...
    try {
      ...
      if (!forceRefresh && subscriptionCache.has(cacheKey)) {
        ...
        if (now - cached.timestamp < cacheExpiry) {
          ...
          setGraceUntil(cached.graceUntil ?? null)
          setLoading(false)
          return
        }
      }
      ...
      if (pendingRequests.has(cacheKey)) {
        ...
        setGraceUntil(result.graceUntil ?? null)
        setLoading(false)
        return
      }
      ...
      const requestPromise = (async () => {
        try {
          ...
          if (subscriptions && subscriptions.length > 0) {
            ...
            // ✅ STATUS-BASED LOGIC - webhook decides everything!
            if (subscription.status === 'trial') {
              // Trial is active until webhook changes it to 'active' or 'expired'
              isActive = true
              console.log('✅ Trial subscription - showing as active (waiting for webhook)')
            }
            ...
            if (isActive && !isExpired) {
              ...
              // ⚠️ REMOVED: handleExpiredSubscription - webhook already set status
          ...
          // Fetch grace_until from majstors if freemium
          let finalGraceUntil = null
          if (!finalSubscription || finalPlan?.name === 'freemium') {
            const { data: majstorRow } = await supabase
              .from('majstors')
              .select('grace_until')
              .eq('id', majstorId)
              .single()
            finalGraceUntil = majstorRow?.grace_until ?? null
          }
  ...
  const _isInGracePeriod = !subscription && plan?.name === 'freemium' && !!graceUntil && new Date(graceUntil) > new Date()
  const _graceDaysRemaining = _isInGracePeriod
    ? Math.max(0, Math.ceil((new Date(graceUntil) - new Date()) / (1000 * 60 * 60 * 24)))
    : 0
  ...
  return {
    ...
    graceDaysRemaining: _graceDaysRemaining,
