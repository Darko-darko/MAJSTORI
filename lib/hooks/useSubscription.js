// lib/hooks/useSubscription.js - SA REALTIME LISTENER 🔥
...
const cacheExpiry = 30 * 1000 // 30 seconds (povećano sa 5s)
...
export function useSubscription(majstorId) {
  ...
  const loadSubscription = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // 🔥 Check cache SAMO ako nije force refresh
      ...
      // 🔥 Ako je force refresh, očisti cache
      if (forceRefresh) {
  ...
  // 🔥🔥🔥 REALTIME LISTENER - OVO JE KLJUČNO! 🔥🔥🔥
  useEffect(() => {
    if (!majstorId) return

    console.log('🔔 Setting up Realtime listener for majstor:', majstorId)

    // Kreiraj channel za slušanje promena
