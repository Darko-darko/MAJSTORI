...
export function useSubscription(majstorId) {
        ...
        try {
          ...
          if (!latestSubscription) {
            ...
            if (majstorDataResult?.subscription_status === 'trial') {
              // 7-day trial - nema plan, samo trial status
  ...
  const isInTrial = useCallback(() => {
    // 🔥 NOVO: Trial može biti i bez subscription unosa (7-day trial)
    if (!subscription && plan === null && majstorData?.subscription_status === 'trial') {
      return true
    }
  ...
  const isActive = useCallback(() => {
    // 🔥 NOVO: 7-day trial (bez subscription) je aktivan
    if (!subscription && majstorData?.subscription_status === 'trial') {
