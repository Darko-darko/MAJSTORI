// lib/hooks/useSubscription.js - TRIAL SUPPORT
'use client'
...
export function useSubscription(majstorId) {
      ...
      const requestPromise = (async () => {
        try {
          ...
          if (!latestSubscription) {
            ...
            // 🔥 NOVA LOGIKA: Proveri status i expiration
            const now = new Date()
            ...
            // Ako je aktivan (bilo trial, paid, ili cancelled ali još važi)
  ...
  // 🔥 ISPRAVLJENA LOGIKA: trialDaysRemaining
  ...
  const isActive = useCallback(() => {
    ...
    // Aktivan ako je:
    // 1. Trial i period nije istekao
    // 2. Active i period nije istekao
    // 3. Cancelled ali period NIJE istekao (grace!)
  ...
  return {
    ...
    isPaid: subscription?.status === 'active', // Stvarno platio
