// lib/hooks/useSubscription.js - FIXED DAYS CALCULATION
'use client'
...
export function useSubscription(majstorId) {
  ...
  const [majstorData, setMajstorData] = useState(null)
    ...
    try {
      ...
      if (!forceRefresh) {
        ...
        if (cached && (Date.now() - cached.timestamp < cacheExpiry)) {
          ...
          setMajstorData(cached.majstorData)
          setLoading(false)
          return
        }
      } else {
      ...
      if (pendingRequests.has(cacheKey) && !forceRefresh) {
        ...
        setMajstorData(result.majstorData)
        setLoading(false)
        return
      }
  ...
  const hasFeatureAccess = useCallback((featureKey) => {
    // Trial ima pristup SVIM features
  ...
  // 🔥 FIXED: Koristi Math.floor() umesto Math.ceil() za tačan broj CELIH dana
  const getTrialDaysRemaining = useCallback(() => {
    // 7-day trial (bez subscription unosa)
    if (!subscription && majstorData?.subscription_status === 'trial' && majstorData?.subscription_ends_at) {
      ...
      // 🔥 Math.floor = zaokruživanje NA DOLE (CELI preostali dani)
