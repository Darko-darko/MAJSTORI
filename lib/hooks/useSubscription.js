// lib/hooks/useSubscription.js - FIXED INFINITE LOOP BUG 🔥
...
export function useSubscription(majstorId) {
  ...
  // 🔥 FIX: useRef za Realtime channel da spreči re-kreiranje
  const realtimeChannelRef = useRef(null)
  const isRealtimeSetup = useRef(false)
  ...
  // 🔥🔥🔥 REALTIME LISTENER - FIXED INFINITE LOOP! 🔥🔥🔥
  useEffect(() => {
    if (!majstorId) return
    
    // 🔥 Spreči kreiranje više channel-ova
    if (isRealtimeSetup.current) {
      console.log('⏭️ Realtime already setup, skipping...')
      return
    }

    console.log('🔔 Setting up Realtime listener for majstor:', majstorId)
    isRealtimeSetup.current = true
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // 🔥 Direktno pozovi loadSubscription
          loadSubscription(true)

          // 🎉 Dispatch celebration event za TRIAL → ACTIVE
          ...
          // 📢 Dispatch generic subscription-changed event
    ...
    return () => {
      ...
      isRealtimeSetup.current = false
      if (realtimeChannelRef.current) {
        realtimeChannelRef.current.unsubscribe()
      }
    }
  }, [majstorId]) // 🔥 SAMO majstorId u dependency!
