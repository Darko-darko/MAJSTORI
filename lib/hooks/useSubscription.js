// lib/hooks/useSubscription.js - OPTIMIZOVANO (bez tihog refresh-a)
...
export function useSubscription(majstorId) {
      ...
      const requestPromise = (async () => {
        try {
          ...
          if (!latestSubscription) {
            ...
            // Normal status checks if not cancelled_at
            if (!subscription.cancelled_at) {
  ...
  // 🔥 UKLONJEN TIHI REFRESH NAKON paddle_success
  // Sada dashboard page radi vizuelni refresh sa progress modal-om

  // Periodic background refresh (svaka 3 minuta)
  useEffect(() => {
    if (!majstorId) return

    const interval = setInterval(() => {
      console.log('🔄 Periodic subscription refresh (background)')
