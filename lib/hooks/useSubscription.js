// lib/hooks/useSubscription.js - SIMPLE SMART REFRESH
'use client'
 ...
 * - Odmah nakon plaćanja: JEDAN refresh nakon 3 sekunde
 * - Normalno: refresh svakih 3 minuta
 */
const INITIAL_REFRESH_DELAY = 1000 // 1 sekunde nakon plaćanja
const SLOW_REFRESH_INTERVAL = 60 * 1000 // 60 sec

/**
 * Hook za upravljanje pretplatama korisnika
 * FIXED: Simple refresh - samo jedan brzi nakon plaćanja, pa sporo
 */
export function useSubscription(majstorId) {
  ...
  useEffect(() => {
    ...
    if (isJustPaid && !initialRefreshDoneRef.current) {
      // 🚀 INITIAL REFRESH - samo jedan put nakon 3 sekunde
      console.log('💳 Payment detected - scheduling ONE refresh after 3 seconds')
      ...
      const timeoutId = setTimeout(() => {
        ...
        console.log('🐌 Starting SLOW refresh mode (45 sec)')
...
export function markPaymentJustCompleted() {
  if (typeof window !== 'undefined') {
    ...
    console.log('💳 Payment marked - ONE refresh in 3 seconds')
  }
}
