...
export class ZUGFeRDService {
  ...
  static generateZUGFeRDXML(invoiceData, majstorData) {
    try {
      ...
      if (invoiceData.type === 'storno') {
        console.log('🇪🇺 Generating ZUGFeRD 2.4 type 381 (Credit Note) for:', invoiceData.invoice_number)
      }
...
<rsm:CrossIndustryInvoice xmlns:ram="urn:un:unece:uncefact:data:standard:ReusableAggregateBusinessInformationEntity:100" 
  ...
  static buildExchangedDocument(invoice, documentId, documentDate) {
    ...
    <ram:TypeCode>${invoice.type === 'storno' ? '381' : '380'}</ram:TypeCode>
    <ram:IssueDateTime>
...
if (taxCategory === 'Z' || invoice.is_kleinunternehmer) vatRate = 0
  ...
  static buildApplicableHeaderTradeSettlement(invoice, majstor) {
    ...
    // Parse subtotal and total safely — abs() for storno (ZUGFeRD 381 uses positive amounts)
    const subtotal = Math.abs(this.parseNumber(invoice.subtotal, 0))
    const totalAmount = Math.abs(this.parseNumber(invoice.total_amount, 0))
    const effectiveTaxAmount = Math.abs(this.parseNumber(invoice.tax_amount, 0))
    let effectiveVatRate = this.parseNumber(invoice.tax_rate, 0)

    // Rabatt, Skonto, Einbehalt
    const rabattPct = this.parseNumber(invoice.rabatt_percent, 0)
    const rabattAmount = this.parseNumber(invoice.rabatt_amount, 0)
    const rabattReason = invoice.rabatt_reason || 'Rabatt'
    const skontoPct = this.parseNumber(invoice.skonto_percent, 0)
    const skontoDays = parseInt(invoice.skonto_days) || 0
    const einbehaltPct = this.parseNumber(invoice.sicherheitseinbehalt_percent, 0)
    const einbehaltYears = parseInt(invoice.sicherheitseinbehalt_years) || 2
    const taxBasis = rabattAmount > 0 ? parseFloat((subtotal - rabattAmount).toFixed(2)) : subtotal
    ...
    return `    <ram:ApplicableHeaderTradeSettlement>
      ...
      </ram:ApplicableTradeTax>${rabattAmount > 0 ? `
      <ram:SpecifiedTradeAllowanceCharge>
        <ram:ChargeIndicator><udt:Indicator>false</udt:Indicator></ram:ChargeIndicator>
        <ram:CalculationPercent>${rabattPct.toFixed(2)}</ram:CalculationPercent>
        <ram:BasisAmount>${subtotal.toFixed(2)}</ram:BasisAmount>
        <ram:ActualAmount>${rabattAmount.toFixed(2)}</ram:ActualAmount>
        <ram:Reason>${this.escapeXML(rabattReason)}</ram:Reason>
        <ram:CategoryTradeTax>
          <ram:TypeCode>VAT</ram:TypeCode>
          <ram:CategoryCode>${taxCategory}</ram:CategoryCode>
          <ram:RateApplicablePercent>${effectiveVatRate.toFixed(2)}</ram:RateApplicablePercent>
        </ram:CategoryTradeTax>
      </ram:SpecifiedTradeAllowanceCharge>` : ''}${this.buildPaymentTerms(invoice, skontoPct, skontoDays, einbehaltPct, einbehaltYears)}
      <ram:SpecifiedTradeSettlementHeaderMonetarySummation>
        <ram:LineTotalAmount>${subtotal.toFixed(2)}</ram:LineTotalAmount>${rabattAmount > 0 ? `
        <ram:AllowanceTotalAmount>${rabattAmount.toFixed(2)}</ram:AllowanceTotalAmount>` : ''}
      ...
      </ram:SpecifiedTradeSettlementHeaderMonetarySummation>${isStorno && invoice.stornoOfNumber ? `
      <ram:InvoiceReferencedDocument>
        <ram:IssuerAssignedID>${this.escapeXML(invoice.stornoOfNumber)}</ram:IssuerAssignedID>
      </ram:InvoiceReferencedDocument>` : ''}
   ...
   */
  static buildPaymentTerms(invoice, skontoPct = 0, skontoDays = 0, einbehaltPct = 0, einbehaltYears = 2) {
    ...
    // Build description with skonto + einbehalt info
    let desc = days === 0 ? 'Zahlbar sofort' : `Zahlbar innerhalb ${days} Tagen`
    if (skontoPct > 0) {
      desc += `. ${skontoPct}% Skonto bei Zahlung innerhalb ${skontoDays} Tagen`
    }
    if (einbehaltPct > 0) {
      const einbehaltDate = new Date(invoice.issue_date || new Date())
      einbehaltDate.setFullYear(einbehaltDate.getFullYear() + einbehaltYears)
      desc += `. ${einbehaltPct}% Sicherheitseinbehalt, faellig am ${einbehaltDate.toLocaleDateString('de-DE')}`
    }

    return `
      <ram:SpecifiedTradePaymentTerms>
        <ram:Description>${this.escapeXML(desc)}</ram:Description>
        <ram:DueDateDateTime>
  ...
  static getUnitCode(unit) {
    const map = { 'm²': 'MTK', 'lfm': 'MTR', 'm³': 'MTQ', 'Stk': 'C62', 'Std': 'HUR', 'pausch': 'LS', 'L': 'LTR', 'kg': 'KGM', 'Karton': 'CT', 'Sack': 'SA' }
    return map[unit] || 'C62'
  }
  ...
  static canGenerateZUGFeRD(invoiceData, majstorData) {
    ...
    if (invoiceData.type !== 'invoice' && invoiceData.type !== 'storno') {
      missingFields.push('Must be invoice or storno (not quote)')
    }
