...
static buildApplicableHeaderTradeAgreement(invoice, majstor) {
  return `    <ram:ApplicableHeaderTradeAgreement>
    ...
    <ram:BuyerTradeParty>
      ...
      ${this.buildBuyerTaxRegistration(invoice)}
    </ram:BuyerTradeParty>
  ...
  /**
 * Buyer tax registration (for B2B invoices)
 * @param {Object} invoice - Invoice data with customer_tax_number
 * @returns {string} XML buyer tax registration
 */
static buildBuyerTaxRegistration(invoice) {
  // Only include if customer has tax number (B2B scenario)
  if (!invoice.customer_tax_number || !invoice.customer_tax_number.trim()) {
    return '' // B2C - no tax registration needed
  }

  const taxNumber = invoice.customer_tax_number.trim()
  
  // Check if it's a VAT ID (starts with country code) or local tax number
  const isVATID = /^[A-Z]{2}/.test(taxNumber)
  
  if (isVATID) {
    // It's a VAT ID (USt-IdNr) - use VA scheme
    return `      <ram:SpecifiedTaxRegistration>
        <ram:ID schemeID="VA">${this.escapeXML(taxNumber)}</ram:ID>
      </ram:SpecifiedTaxRegistration>`
  } else {
    // It's a local tax number (Steuernummer) - use FC scheme
    return `      <ram:SpecifiedTaxRegistration>
