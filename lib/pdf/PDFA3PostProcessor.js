...
export class PDFA3PostProcessor {
  ...
  static async inject(pdfBuffer, invoice, majstor) {
    try {
      ...
      // ── 4. AF (AssociatedFiles) array — required for PDF/A-3 + Mustang ────────
      // pdfkit adds factur-x.xml to EmbeddedFiles name tree but NOT to Catalog.AF.
      // Without AF, Mustang cannot find the attachment and reports PDF:invalid.
      try {
        ...
        if (nameArr instanceof PDFArray) {
          for (let i = 0; i + 1 < nameArr.size(); i += 2) {
            ...
            const nameStr = nameObj?.decodeText?.() ?? nameObj?.asString?.() ?? ''
            if (nameStr === ZUGFERD.xmlFilename) {
              const filespecRef = nameArr.get(i + 1)       // keep as PDFRef
              // Ensure AFRelationship is set on the filespec dict
              const filespecDict = context.lookup(filespecRef)
              if (filespecDict instanceof PDFDict
                  && !filespecDict.has(PDFName.of('AFRelationship'))) {
                filespecDict.set(PDFName.of('AFRelationship'), PDFName.of('Data'))
              }
              // Add Catalog.AF → [filespecRef]
        ...
        console.warn('⚠️  AF array injection failed (non-fatal):', e.message)
      }
  ...
  static buildXMP(invoice, majstor) {
    const docType  = invoice.type === 'quote' ? 'Angebot' : invoice.type === 'storno' ? 'Stornorechnung' : 'Rechnung'
    const docNum   = invoice.invoice_number || invoice.quote_number || ''
    const title    = `${docType} ${docNum}`.trim()
