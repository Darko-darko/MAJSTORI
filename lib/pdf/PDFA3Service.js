...
<?xpacket end="w"?>`
  ...
  static injectXMP(doc, xmpContent) {
    ...
    metaStream.compress = false  // XMP must be plain text — no FlateDecode
  ...
  static injectOutputIntent(doc) {
    ...
    try {
      ...
      iccStream = doc.ref({ N: 3 })  // no Length — pdfkit calculates it
      iccStream.compress = false     // ICC binary must NOT be recompressed
