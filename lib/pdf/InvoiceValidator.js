...
const VALID_UNIT_CODES = new Set(['C62', 'MTK', 'MTR', 'MTQ', 'HUR', 'LS', 'LTR', 'KGM', 'CT', 'SA', ''])
...
export function validateForZUGFeRD(normalized, majstorData) {
  ...
  if (!ZUGFERD.profileURN) {
    errors.push('BT-23: profileURN fehlt in ZUGFeRDConfig')
  }
  ...
  items.forEach((item, i) => {
    ...
    const unitMap = { 'm²': 'MTK', 'lfm': 'MTR', 'm³': 'MTQ', 'Stk': 'C62', 'Std': 'HUR', 'pausch': 'LS', 'L': 'LTR', 'kg': 'KGM', 'Karton': 'CT', 'Sack': 'SA' }
  ...
  // Amount consistency check (accounting for Rabatt)
  const rabattAmount = round2(parseFloat(normalized.rabatt_amount) || 0)
  const nettoAfterRabatt = round2(normalized.subtotal - rabattAmount)
  const diff = Math.abs(round2(nettoAfterRabatt + normalized.taxAmount) - normalized.totalAmount)
  if (diff > 0.02) {
    errors.push(
      `Betragsinkonsistenz: Netto (${normalized.subtotal}) - Rabatt (${rabattAmount}) + MwSt (${normalized.taxAmount}) ≠ Brutto (${normalized.totalAmount}), Differenz: ${diff.toFixed(2)}`
    )
  }
