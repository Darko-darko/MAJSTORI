...
export function validateForZUGFeRD(normalized, majstorData) {
  ...
  const rabattAmount = parseFloat(normalized.rabatt_amount) || 0
  const nettoAfterRabatt = normalized.subtotal - rabattAmount
  const diff = Math.abs(nettoAfterRabatt + normalized.taxAmount - normalized.totalAmount)
  if (diff > 0.02) {
    errors.push(
