...
function buildDoc(jsPDF, aufmass, majstor, signatureDataUrl, includeSignatureSection) {
  ...
  for (const room of rooms) {
    ...
    // Maler: sort items — Wandfläche first, then Abzüge, then rest
    const isMaler = aufmass.gewerk === 'maler'
    const sortedItems = isMaler
      ? [
          ...items.filter(i => i.unit === 'Wand' && !i.subtract),
          ...items.filter(i => i.subtract),
          ...items.filter(i => i.unit !== 'Wand' && !i.subtract && !i.isForm),
    ...
    for (const item of sortedItems) {
      ...
      if (isAbzug && vobThr > 0 && item.result > 0) {
        ...
        if (singleArea < vobThr) uebermHint = ' (überm.)'
      }
      const desc = (item.description || '—') + (isAbzug ? ` (Abzug${uebermHint})` : isForm ? ' (Ergänzung)' : '')
    ...
    if (subtractItems.length > 0) {
      if (isMaler) {
        ...
        const decke = items.filter(i => i.unit === 'm²' && !i.subtract && !i.isForm).reduce((s, i) => s + (i.result || 0), 0)
        ...
        if (decke > 0) { doc.text('Deckenfläche:', col.bez, y); doc.text(`${formatNum(decke)} m²`, col.erg, y); y += 5 }
        if (lfm > 0) { doc.text('Sockelleiste:', col.bez, y); doc.text(`${formatNum(lfm)} lfm`, col.erg, y); y += 5 }
        if (stk > 0) { doc.text('Stück:', col.bez, y); doc.text(`${formatNum(stk)} Stk`, col.erg, y); y += 5 }
        doc.setFont('helvetica', 'normal')
      } else {
        // Generic: single-line netto
        const nettoByUnit = {}
...
function drawRoomSketch(doc, room, margin, startY, contentW, gewerk) {
  ...
  // For Maler: always use Wand sketch (Decke is m² but wall sketch is the standard)
