        ...
        for (const si of sortedIdxs) {
          ...
          if (hasBreakdown) {
            ...
            if (olDispMm > 0) {
              ...
              doc.text(`${olDispMm}`, bdX + 4, sy + olH / 2, { align: 'center', angle: 90 })
            }
            doc.line(bdX - 1, sy + olH, bdX + 1, sy + olH)
            doc.line(bdX - 1, sy + sh - ulH, bdX + 1, sy + sh - ulH)
            doc.line(bdX, sy + olH, bdX, sy + sh - ulH)
            doc.text(`${fluegelHmm}`, bdX + 4, sy + olH + (sh - olH - ulH) / 2, { align: 'center', angle: 90 })
            if (ulDispMm > 0) {
              doc.line(bdX - 1, sy + sh - ulH, bdX + 1, sy + sh - ulH)
              doc.line(bdX - 1, sy + sh, bdX + 1, sy + sh)
              doc.line(bdX, sy + sh - ulH, bdX, sy + sh)
              doc.text(`${ulDispMm}`, bdX + 4, sy + sh - ulH + ulH / 2, { align: 'center', angle: 90 })
            }
            doc.setFontSize(6)
            colIdx--
          }
