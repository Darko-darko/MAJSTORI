...
function buildDoc(jsPDF, aufmass, majstor, signatureDataUrl, includeSignatureSection) {
  ...
  for (const room of rooms) {
    ...
    for (const item of sortedItems) {
      ...
      const resultStr = item.result != null ? (isAbzug ? (isUebermessen ? formatNum(item.result) : `- ${formatNum(item.result)}`) : isForm ? `+ ${formatNum(item.result)}` : formatNum(item.result)) : '-'
    ...
    if (subtractItems.length > 0) {
      if (isMaler) {
        ...
        const flaeche = items.filter(i => i.unit === 'm²' && !i.subtract && !i.isForm).reduce((s, i) => s + (i.result || 0), 0)
...
function drawRoomSketch(doc, room, margin, startY, contentW, gewerk) {
  ...
  const wallTrades = ['maler', 'fliesen', 'trockenbau']
  ...
  const m2Openings = (room.items || []).filter(i => i.subtract && i.length && i.unit === 'm²')
  ...
  if (nGroups > 0) {
    ...
    for (let gi = 0; gi < nGroups; gi++) {
      ...
      // Label above opening
      const dimLabel = (totalCount > 1 ? `${totalCount}× ` : '') + (op.description || 'Abzug')
      ...
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(5)
      const dimSub = isRound ? `⌀${fmtDim(oL)}` : (oHraw > 0 ? `${fmtDim(oL)}×${fmtDim(oHraw)}` : fmtDim(oL))
