...
function buildDoc(jsPDF, aufmass, majstor, signatureDataUrl, includeSignatureSection) {
  ...
  for (const room of rooms) {
    ...
    if (subtractItems.length > 0) {
      const nettoByUnit = {}
      // For Maler: keep Wand separate from Decke (m²), subtract openings only from Wand
      const isMaler = aufmass.gewerk === 'maler'
      for (const item of items) {
        if (item.result == null) continue
        const rawU = item.unit || ''
        // Maler: separate 'Wand' from 'm²' (Decke) so Abzug only applies to Wand
        const u = isMaler
          ? (['Bogen', 'Trap'].includes(rawU) ? 'm²' : rawU)
          : (['Wand', 'Bogen', 'Trap'].includes(rawU) ? 'm²' : rawU)
        if (!u) continue
        let sign = item.subtract ? -1 : 1
        // VOB Übermessung: opening < threshold → don't subtract
        if (item.subtract && vobThr > 0) {
          const cnt = parseFloat(item.count) || 1
          const singleArea = cnt > 0 ? item.result / cnt : item.result
          if (singleArea < vobThr) sign = 0
        }
        // Maler: openings subtract from Wand, not from m² (Decke)
        if (isMaler && item.subtract) {
          nettoByUnit['Wand'] = (nettoByUnit['Wand'] || 0) + item.result * sign
        } else {
      ...
      // Display: normalize Wand back to m² label for display
      const nettoStr = Object.entries(nettoByUnit)
        .filter(([, v]) => Math.abs(v) > 0.001)
        .map(([u, v]) => `${formatNum(v)} ${u === 'Wand' ? 'm² (Wand)' : u}`)
        .join(' | ')
