...
function buildDoc(aufmass, majstor, signatureDataUrl, includeSignatureSection) {
  ...
  for (const room of rooms) {
    ...
    for (const item of items) {
      ...
      const result = item.result != null ? formatNum(item.result) : '—'
...
function drawRoomSketch(doc, room, margin, startY, contentW) {
  ...
  const hasRect = (room.items || []).some(i =>
    (i.unit === 'm²' || i.unit === 'Wand') && i.length && i.width
  )
  ...
  const hasWand = (room.items || []).some(i => i.unit === 'Wand' && i.length && i.width && i.height)
  const hasM2   = (room.items || []).some(i => i.unit === 'm²' && i.length && i.width)
  if (hasWand && !hasM2) {
  ...
  for (const room of rooms || []) {
    for (const item of room.items || []) {
      if (item.unit && item.result != null) {
        ...
        totals[u] = (totals[u] || 0) + item.result
      }
    }
  }
  return totals
