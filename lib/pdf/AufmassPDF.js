  ...
  if (aufmass.gewerk === 'fensterbau') {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        for (let si = 0; si < segs.length; si++) {
          const sw = segWidths[si] * scale
          doc.line(cx2, sketchY + sketchH + 1, cx2, dimY1 + 1.5)
          doc.line(cx2 + sw, sketchY + sketchH + 1, cx2 + sw, dimY1 + 1.5)
          doc.line(cx2, dimY1, cx2 + sw, dimY1)
          doc.line(cx2, dimY1 - 1, cx2, dimY1 + 1)
          doc.line(cx2 + sw, dimY1 - 1, cx2 + sw, dimY1 + 1)
          doc.text(`${segWidths[si]}`, cx2 + sw / 2, dimY1 + 3.5, { align: 'center' })
          cx2 += sw
        }
        // Total width
        if (segs.length > 1) {
          const totalDimY = dimY1 + 6
          doc.line(sketchX, dimY1 + 2, sketchX, totalDimY + 1.5)
          doc.line(sketchX + sketchW, dimY1 + 2, sketchX + sketchW, totalDimY + 1.5)
        ...
        // Right: height of rightmost segment
        ...
        const dimX = sketchX + sketchW + 4
        doc.line(sketchX + sketchW + 1, lastSY, dimX + 1.5, lastSY)
        doc.line(sketchX + sketchW + 1, lastSY + lastSH, dimX + 1.5, lastSY + lastSH)
        doc.line(dimX, lastSY, dimX, lastSY + lastSH)
        doc.line(dimX - 1, lastSY, dimX + 1, lastSY)
        doc.line(dimX - 1, lastSY + lastSH, dimX + 1, lastSY + lastSH)
        doc.text(`${lastH}`, dimX + 3, lastSY + lastSH / 2, { align: 'center', angle: 90 })

        // "Ansicht von innen"
        doc.setFontSize(6); doc.setTextColor(150)
        doc.text('Ansicht von innen', sketchX + sketchW / 2, sketchY + sketchH + (segs.length > 1 ? 18 : 10), { align: 'center' })
