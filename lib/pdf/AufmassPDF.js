...
export async function generateAufmassPDF(aufmass, majstor, signatureDataUrl = null) {
  ...
  const doc = buildDoc(jsPDF, aufmass, majstor, signatureDataUrl, true)
...
export async function generateAufmassPDFBlob(aufmass, majstor) {
  const { default: jsPDF } = await import('jspdf')
  const doc = buildDoc(jsPDF, aufmass, majstor, null, false)
...
function buildDoc(jsPDF, aufmass, majstor, signatureDataUrl, includeSignatureSection) {
  ...
  if (aufmass.gewerk === 'fensterbau') {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      const totalPW = hasCustomWidths ? panels.reduce((s, p) => s + (parseFloat(p.width) || 1), 0) : panels.length
      const panelWidths = panels.map(p => hasCustomWidths ? (parseFloat(p.width) || 1) / totalPW * sketchW : sketchW / panels.length)

      // Oberlicht
      if (pos.oberlicht) {
        doc.setDrawColor(80)
        doc.line(sketchX, sketchY + olH, sketchX + sketchW, sketchY + olH)
        // Fix cross in oberlicht
        doc.setDrawColor(160)
        doc.setLineDashPattern([1.5, 1.5], 0)
        doc.line(sketchX, sketchY, sketchX + sketchW, sketchY + olH)
        doc.line(sketchX + sketchW, sketchY, sketchX, sketchY + olH)
      ...
      for (let i = 0; i < panels.length; i++) {
        ...
        if (p.type === 'fix') {
          doc.setDrawColor(160)
          doc.setLineDashPattern([1.5, 1.5], 0)
          doc.line(px, panelY, px + pw, panelY + panelH)
          doc.line(px + pw, panelY, px, panelY + panelH)
          doc.setLineDashPattern([], 0)
        }
        if (p.type === 'kipp' || p.type === 'kipp-dreh') {
          doc.setDrawColor(160)
          doc.setLineDashPattern([1.5, 1.5], 0)
          doc.line(px, panelY + panelH, cx, panelY)
          doc.line(px + pw, panelY + panelH, cx, panelY)
          doc.setLineDashPattern([], 0)
        }
        if (p.type === 'dreh' || p.type === 'kipp-dreh') {
          ...
          if (p.hinge === 'left' || !p.hinge) {
            doc.line(px, panelY, px + pw, cy)
            doc.line(px, panelY + panelH, px + pw, cy)
          } else {
            doc.line(px + pw, panelY, px, cy)
            doc.line(px + pw, panelY + panelH, px, cy)
          }
        }
        pxOff += pw
      }

      // Dimensions below sketch
      if (pos.width || pos.height) {
        doc.setFontSize(8)
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(100)
        const dimText = `${pos.width || '?'} × ${pos.height || '?'} mm`
        doc.text(dimText, sketchX + sketchW / 2, sketchY + sketchH + 5, { align: 'center' })
      }

      // Right side: details table
      const detX = margin + 62
      const detW = contentW - 62
      doc.setFontSize(8)
      doc.setTextColor(80)
      let dy = sketchY
      ...
      // Preset label
      const presetLabel = pos.preset ? (() => {
        const LABELS = { '1-kd-l': '1-flg. Dreh-Kipp', '2-kd': '2-flg. Dreh-Kipp', '1-fix': 'Festverglasung', '1-kipp': 'Kippfenster', 'kd-fix': 'Dreh-Kipp + Fest', '3-kd': '3-flügelig', '1-kd-ob': 'DK + Oberlicht', '2-kd-ob': '2-flg. + Oberlicht' }
        return LABELS[pos.preset] || pos.preset
      })() : null

      if (presetLabel) {
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(30)
        doc.text(presetLabel, detX, dy)
        dy += 5
      }
      ...
      for (const [label, value] of details) {
        ...
        doc.setFont('helvetica', 'normal')
        const valLines = doc.splitTextToSize(value, detW - 30)
        doc.text(valLines, detX + 28, dy)
        dy += valLines.length * 4 + 1
      }

      y = Math.max(y, sketchY + sketchH + 8, dy + 4)
    ...
    if (positions.length > 0) {
      ...
      doc.text(`Gesamt: ${totalCount} Fenster/Türen`, margin, y)
      y += 8
    }
  } else {
