...
export class InvoicePDFService {
  ...
  addDIN5008AddressField(invoiceData, majstorData) {
    ...
    if (invoiceData.customer_postal_code && invoiceData.customer_city) {
      // Always show country from database (plain text, no emoji)
      const country = invoiceData.customer_country || 'Deutschland'
      const cityLine = `${invoiceData.customer_postal_code} ${invoiceData.customer_city}, ${country}`
    ...
    if (invoiceData.weg_street) {
      ...
      if (invoiceData.weg_postal_code && invoiceData.weg_city) {
        // Always show country from database (plain text, no emoji)
        const wegCountry = invoiceData.weg_country || 'Deutschland'
        const wegCityLine = `${invoiceData.weg_postal_code} ${invoiceData.weg_city}, ${wegCountry}`
  ...
  addPageFooter(majstorData, isLastPage = false, hasZUGFeRD = false) {
    ...
    if (isLastPage && hasZUGFeRD) {
      ...
      this.doc.fillColor(this.COLORS.accent)
      this.doc.text('âœ” ZUGFeRD 2.1 KONFORM - Enthält maschinenlesbare XML-Rechnungsdaten', 50, zugferdY, { 
        width: 500, 
        align: 'left' 
      })
      this.doc.fillColor('#000000')
