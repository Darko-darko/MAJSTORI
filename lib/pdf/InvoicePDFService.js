  ...
  async addTotalsSection(invoiceData, majstorData) {
    ...
    if (rabattPct > 0) {
      ...
      const rabattLabel = rabattReason
        ...
        : `Rabatt ${rabattPct}%:`
      this.doc.text(rabattLabel, labelX, y)
    ...
    // Gesamtbetrag box
    const boxWidth = valueX + 75 - labelX + 10
    this.doc.save()
    this.doc.rect(labelX - 4, y - 2, boxWidth, 24)
       .fillOpacity(0.03).fill('#000000').fillOpacity(1)
    this.doc.rect(labelX - 5, y - 3, boxWidth, 24)
       .lineWidth(2).strokeColor(this.COLORS.success).stroke()
    this.doc.rect(labelX - 5, y - 3, boxWidth, 3)
