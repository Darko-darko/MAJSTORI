...
export class InvoicePDFService {
  ...
  // ✅ Header sa logom DESNO (ZADRŽANO) - optimized
  async addDIN5008Header(majstorData, isFirstPage = false) {
    if (isFirstPage) {
      // 📄 PRVA STRANICA
      let y = 20
      
      // 🖼️ LOGO DESNO (kao Dilber) - VEĆI i lepši
      const logoX = 385
      const logoY = 15
      if (majstorData.business_logo_url) {
        try {
          await this.addLogoRight(majstorData.business_logo_url, logoX, logoY)
          y = Math.max(y + 85, this.currentY)
        } catch (logoError) {
          console.warn('⚠️ Logo load failed:', logoError.message)
        }
      }
      
      // 📋 BIZNIS INFO - elegantni okvir ispod loga
      const infoX = 385
      let infoY = logoY + 85
      ...
      this.currentY = 200
      console.log('✅ Header (first page) added with logo right')
  ...
  // ✅ Logo desno - OPTIMIZED (ZADRŽAN DESNO!)
  async addLogoRight(logoUrl, logoX, logoY) {
    try {
      ...
      console.log('✅ Logo added (right, 180×70px)')
    } catch (error) {
  ...
  async addInvoiceFooterSection(majstorData) {
    const footerText = majstorData.invoice_footer_text || 
