...
this.doc.text(
  ...
  async addAufmassPositions() {
    const aufmass = this.aufmassData
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      // Estimate height: header(22) + sketch(75) + kote(30) + padding(20) = ~150
      // But details (especially Bemerkung) can be taller, so use generous estimate
      const estimatedH = 170
      await this.checkSpaceAndAddPageIfNeeded(estimatedH)
      y = this.currentY > y ? this.currentY : y
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        for (const si of sortedIdxs) {
          ...
          const dimX = sketchX + sketchW + 6 + colIdx * colSp
          ...
          this.doc.save()
          this.doc.translate(dimX + 10, sy + sh)
          this.doc.rotate(-90)
          ...
          if (hasBreakdown) {
            ...
            const bdX = sketchX + sketchW + 6 + colIdx * colSp
            ...
            if (olDispMm > 0) {
              ...
              this.doc.save()
              this.doc.translate(bdX + 10, sy + olH2)
              this.doc.rotate(-90)
            ...
            this.doc.save()
            this.doc.translate(bdX + 10, sy + sh - ulH2)
            this.doc.rotate(-90)
            ...
            if (ulDispMm > 0) {
              ...
              this.doc.save()
              this.doc.translate(bdX + 10, sy + sh)
              this.doc.rotate(-90)
        ...
        // Details — offset past all dim columns
        const dimRightEdge = sketchX + sketchW + 6 + (dimColsR - 1) * colSp + 14
        const detX = dimRightEdge + 3
        const detValX = detX + 60
        this.setFont('regular', this.FONT_SIZES.small)
        this.doc.fillColor('#333333')
        let dy = sketchY
        ...
        const detW = Math.max(80, this.doc.page.width - this.doc.page.margins.right - detValX)
        for (const [label, value] of details) {
          if (dy > 720) break // prevent footer overflow
          this.setFont('bold', this.FONT_SIZES.small)
          this.doc.text(label, detX, dy, { lineBreak: false })
          this.setFont('regular', this.FONT_SIZES.small)
          let valStr = String(value || '')
          // Limit Bemerkung to max 3 lines to prevent overflow
          if (label === 'Bemerkung:') {
            const maxH = 36
            while (valStr.length > 10 && this.doc.heightOfString(valStr, { width: detW }) > maxH) {
              valStr = valStr.slice(0, -10) + '...'
            }
          }
          const valH = this.doc.heightOfString(valStr, { width: detW })
          this.doc.text(valStr, detValX, dy, { width: detW })
          dy += Math.max(13, valH + 3)
        }
      ...
      // Left: window sketch — proportional
      ...
      const sketchX = 60
      const sketchY = y
      const inset = 3 // frame inset for Flügelrahmen
      const handleW = 1.5, handleH = 6
      ...
      if (totalPosH > 0) {
        ...
        const dimX = sketchX + sketchW + 6 + totalColIdx * colSpH
        ...
        this.doc.save()
        this.doc.translate(dimX + 10, sketchY + sketchH)
        this.doc.rotate(-90)
        ...
        if (hasHeightBreakdown) {
          ...
          if (olDispMm > 0) {
            ...
            this.doc.save()
            this.doc.translate(bdX + 10, sketchY + olH)
            this.doc.rotate(-90)
          ...
          this.doc.save()
          this.doc.translate(bdX + 10, sketchY + sketchH - ulH)
          this.doc.rotate(-90)
          ...
          if (ulDispMm > 0) {
            ...
            this.doc.save()
            this.doc.translate(bdX + 10, sketchY + sketchH)
            this.doc.rotate(-90)
      ...
      // Right side: details — offset past dim columns
      const dimRightEdgeStd = sketchX + sketchW + 6 + (heightDimCols - 1) * colSpH + 14
      const detX = dimRightEdgeStd + 3
      const detValX = detX + 60
      this.setFont('regular', this.FONT_SIZES.small)
      this.doc.fillColor('#333333')
      let dy = sketchY
      ...
      const detW2 = Math.max(80, this.doc.page.width - this.doc.page.margins.right - detValX)
      for (const [label, value] of details) {
        if (dy > 720) break // prevent footer overflow
        this.setFont('bold', this.FONT_SIZES.small)
        this.doc.text(label, detX, dy, { lineBreak: false })
        this.setFont('regular', this.FONT_SIZES.small)
        let valStr = String(value || '')
        // Limit Bemerkung to prevent overflow
        if (label === 'Bemerkung:') {
          const maxH = 36
          while (valStr.length > 10 && this.doc.heightOfString(valStr, { width: detW2 }) > maxH) {
            valStr = valStr.slice(0, -10) + '...'
          }
        }
        const valH = this.doc.heightOfString(valStr, { width: detW2 })
        this.doc.text(valStr, detValX, dy, { width: detW2 })
        dy += Math.max(13, valH + 3)
      }
  ...
  async generateInvoice(invoiceData, majstorData, aufmassData = null) {
