  ...
  async addAufmassPositions() {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        const detW = this.doc.page.width - this.margins.right - detValX
        for (const [label, value] of details) {
          this.setFont('bold', this.FONT_SIZES.small)
          this.doc.text(label, detX, dy, { continued: false })
          this.setFont('regular', this.FONT_SIZES.small)
          const valH = this.doc.heightOfString(value, { width: detW })
          this.doc.text(value, detValX, dy, { width: detW })
      ...
      const detW2 = this.doc.page.width - this.margins.right - detValX
      for (const [label, value] of details) {
        this.setFont('bold', this.FONT_SIZES.small)
        this.doc.text(label, detX, dy, { continued: false })
        this.setFont('regular', this.FONT_SIZES.small)
        const valH = this.doc.heightOfString(value, { width: detW2 })
        this.doc.text(value, detValX, dy, { width: detW2 })
        dy += Math.max(13, valH + 3)
      }
