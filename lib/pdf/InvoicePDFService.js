  ...
  async addAufmassPositions() {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        // Right: per-segment heights sorted by height desc (tallest rightmost)
        const colSp = 14
        ...
        for (const si of sortedIdxs) {
          ...
          // Breakdown column — short ticks only
          if (hasBreakdown) {
            ...
            if (olDispMm > 0) {
              ...
              this.doc.save()
              this.doc.translate(bdX + 8, sy + olH2)
              this.doc.rotate(-90)
            ...
            this.doc.save()
            this.doc.translate(bdX + 8, sy + sh - ulH2)
            this.doc.rotate(-90)
            ...
            if (ulDispMm > 0) {
              ...
              this.doc.save()
              this.doc.translate(bdX + 8, sy + sh)
              this.doc.rotate(-90)
