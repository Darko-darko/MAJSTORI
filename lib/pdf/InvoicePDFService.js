...
export class InvoicePDFService {
  ...
  async generateInvoice(invoiceData, majstorData) {
    ...
    this.addNotesSection(invoiceData)
 ...
 async addTotalsSection(invoiceData, majstorData) {
  ...
  addNotesSection(invoiceData) {
    if (!invoiceData.notes || !invoiceData.notes.trim()) {
      console.log('⏭️ Skipping notes - empty or null')
      return
    }

    console.log('📝 Current Y before notes:', this.currentY)
    console.log('📝 Notes text:', invoiceData.notes)

    let y = this.currentY + 15

    if (y > 670) {
      console.warn('⚠️ Not enough space for notes section, Y:', y)
      return
    }
    ...
    this.currentY = y + textHeight + 10
    
    console.log('✅ Notes section added!')
    console.log('   Text height:', textHeight)
    console.log('   New currentY:', this.currentY)
  }
