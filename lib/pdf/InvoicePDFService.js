...
this.doc.text(
  ...
  async addAufmassPositions() {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      await this.checkSpaceAndAddPageIfNeeded(90)
      ...
      // Frame
      this.doc.save()
      this.doc.rect(sketchX, sketchY, sketchW, sketchH).lineWidth(1).strokeColor('#444444').stroke()
      this.doc.restore()

      const olH = pos.oberlicht ? sketchH * 0.25 : 0
      const panelH = sketchH - olH
      ...
      if (pos.oberlicht) {
        ...
        this.doc.moveTo(sketchX, sketchY).lineTo(sketchX + sketchW, sketchY + olH).lineWidth(0.4).strokeColor('#999999').dash(3, { space: 3 }).stroke()
        this.doc.moveTo(sketchX + sketchW, sketchY).lineTo(sketchX, sketchY + olH).stroke()
        this.doc.restore()
        this.doc.undash()
      }
      ...
      for (let i = 0; i < panels.length; i++) {
        ...
        const cx = px + pw / 2, cy = panelY + panelH / 2

        // Fix — dashed X
        if (p.type === 'fix') {
          this.doc.moveTo(px, panelY).lineTo(px + pw, panelY + panelH).lineWidth(0.4).strokeColor('#999999').dash(3, { space: 3 }).stroke()
          this.doc.moveTo(px + pw, panelY).lineTo(px, panelY + panelH).stroke()
          this.doc.undash()
        }
        // Kipp — dashed triangle from bottom
        if (p.type === 'kipp' || p.type === 'kipp-dreh') {
          this.doc.moveTo(px, panelY + panelH).lineTo(cx, panelY).lineWidth(0.4).strokeColor('#999999').dash(3, { space: 3 }).stroke()
          this.doc.moveTo(px + pw, panelY + panelH).lineTo(cx, panelY).stroke()
          this.doc.undash()
        }
        ...
        if (p.type === 'dreh' || p.type === 'kipp-dreh') {
          ...
          if (p.hinge === 'left' || !p.hinge) {
            this.doc.moveTo(px, panelY).lineTo(px + pw, cy).stroke()
            this.doc.moveTo(px, panelY + panelH).lineTo(px + pw, cy).stroke()
          } else {
            this.doc.moveTo(px + pw, panelY).lineTo(px, cy).stroke()
            this.doc.moveTo(px + pw, panelY + panelH).lineTo(px, cy).stroke()
          }
        }
        this.doc.restore()
        pxOff += pw
      ...
      for (const [label, value] of details) {
        ...
        this.setFont('regular', this.FONT_SIZES.small)
        this.doc.text(value, detX + 65, dy)
        dy += 13
      }

      y = Math.max(sketchY + sketchH + (hasCustomWidths ? 28 : 16), dy + 5)
      this.currentY = y
    }
    ...
    this.setFont('bold', this.FONT_SIZES.small)
    this.doc.fillColor('#333333')
    const totalCount = positions.reduce((s, p) => s + (parseInt(p.count) || 1), 0)
    this.doc.text(`Gesamt: ${totalCount} Fenster/Türen`, 50, this.currentY + 5)
    this.currentY += 20
  }
