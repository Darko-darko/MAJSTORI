...
this.doc.text(
  ...
  // Aufmaß Maler rooms inline (Grundriss + Positionen)
  async addMalerAufmassRooms() {
    const malerList = (this.aufmassDataList || []).filter(a => a.gewerk === 'maler' && (a.rooms || []).length > 0)
    if (malerList.length === 0) return

    const allRooms = []
    for (const aufmass of malerList) {
      for (const room of (aufmass.rooms || [])) {
        allRooms.push({ room, aufmassTitle: aufmass.title })
      }
    }
    ...
    this.setFont('bold', this.FONT_SIZES.normal)
    this.doc.fillColor('#333333')
    this.doc.text('Aufmaß — Räume', 50, y)
    y += 18

    const vobThreshold = 2.5 // VOB/C DIN 18363
    ...
    for (let ri = 0; ri < allRooms.length; ri++) {
      ...
      const positions = items.filter(i => !i.subtract)
      const openings = items.filter(i => i.subtract)

      // Find Wand item for room dimensions (L, B, H)
      let rL = 0, rB = 0, rH = 0
      // First try room-level dimensions
      if (parseFloat(room.length) > 0 && parseFloat(room.width) > 0 && parseFloat(room.height) > 0) {
        rL = parseFloat(room.length); rB = parseFloat(room.width); rH = parseFloat(room.height)
      } else {
        // Fallback: extract from Wand item
        for (const item of items) {
          if (item.unit === 'Wand' && item.length && item.width && item.height) {
            const s = item.dim_unit === 'cm' ? 0.01 : 1
            rL = (parseFloat(item.length) || 0) * s
            rB = (parseFloat(item.width) || 0) * s
            rH = (parseFloat(item.height) || 0) * s
            break
          }
        }
      }

      const hasSketch = rL > 0 && rB > 0 && rH > 0
      const estimatedH = hasSketch ? 160 : 80
      await this.checkSpaceAndAddPageIfNeeded(estimatedH)
      y = this.currentY
      ...
      this.setFont('bold', this.FONT_SIZES.small)
      this.doc.fillColor('#333333')
      this.doc.text(room.name || `Raum ${ri + 1}`, 55, y + 5)
      // Room dimensions in header
      if (hasSketch) {
        this.setFont('regular', this.FONT_SIZES.tiny)
        this.doc.fillColor('#888888')
        this.doc.text(`${rL.toFixed(2)} × ${rB.toFixed(2)} × ${rH.toFixed(2)} m`, 400, y + 6, { width: 140, align: 'right' })
      }
      y += 22

      // Fixed layout constants (same as Fensterbau)
      const SKETCH_CONTAINER_W = 210
      const DET_X = 72 + SKETCH_CONTAINER_W + 5
      const DET_VAL_X = DET_X + 70
      const DET_VAL_W = 595.28 - 56.69 - DET_VAL_X

      const sketchX = 72
      const sketchY = y

      // ── SKETCH: Unfolded wall 2(L+B) × H ──
      if (hasSketch) {
        const perim = 2 * (rL + rB)
        const maxSkW = 150, maxSkH = 70
        const scale = Math.min(maxSkW / perim, maxSkH / rH)
        const skW = perim * scale
        const skH = rH * scale
        const sx = sketchX + (SKETCH_CONTAINER_W - skW) / 2 - 10
        const sy = sketchY

        // Wall rectangle
        this.doc.save()
        this.doc.rect(sx, sy, skW, skH).fillAndStroke('#f6f7fc', '#555555')
        this.doc.restore()

        // Wall dividers: L | B | L | B (dashed)
        const lW = rL * scale, bW = rB * scale
        const dividers = [lW, lW + bW, lW + bW + lW]
        this.doc.save()
        this.doc.strokeColor('#aaaaaa').lineWidth(0.4).dash(2, { space: 1.5 })
        for (const d of dividers) {
          this.doc.moveTo(sx + d, sy).lineTo(sx + d, sy + skH).stroke()
        }
        this.doc.undash()
        this.doc.restore()

        // Section labels (L, B, L, B)
        if (skH >= 14) {
          this.setFont('regular', 6)
          this.doc.fillColor('#aaaaaa')
          const midY = sy + skH / 2 - 3
          const sections = [
            { label: 'L', cx: lW / 2 },
            { label: 'B', cx: lW + bW / 2 },
          ...
          for (const { label, cx } of sections) {
            this.doc.text(label, sx + cx - 3, midY, { width: 6, align: 'center' })
          }
        }

        // Draw openings as hatched cut-outs
        const nOp = Math.min(openings.length, 6)
        if (nOp > 0) {
          const opGap = skW / (nOp + 1)
          for (let oi = 0; oi < nOp; oi++) {
            const op = openings[oi]
            const s2 = op.dim_unit === 'cm' ? 0.01 : 1
            const oL = (parseFloat(op.length) || 0) * s2
            const oHraw = (parseFloat(op.width || op.height) || 0) * s2
            if (oL <= 0) continue
            const oW = Math.min(oL * scale, skW * 0.85 / nOp)
            const oH = Math.min(oHraw > 0 ? oHraw * scale : skH * 0.55, skH * 0.92)
            const cx = sx + opGap * (oi + 1)
            const ox = Math.max(sx + 0.5, Math.min(cx - oW / 2, sx + skW - oW - 0.5))
            const oy = sy + skH - oH

            // White cut-out
            this.doc.save()
            this.doc.rect(ox, oy, oW, oH).fillAndStroke('#ffffff', '#888888')
            this.doc.restore()

            // 45° hatching
            this.doc.save()
            this.doc.strokeColor('#bbbbbb').lineWidth(0.2)
            const step = 2.5
            ...
            const singleArea = (op.result || 0) / cnt
            const isUebermessen = singleArea > 0 && singleArea < vobThreshold
            this.setFont('bold', 5)
            this.doc.fillColor(isUebermessen ? '#22c55e' : '#c8503c')
            const opLabel = (op.description || 'Öffnung') + (cnt > 1 ? ` ×${cnt}` : '')
            this.doc.text(opLabel, ox, oy - 9, { width: oW, align: 'center' })
            this.setFont('regular', 4.5)
            const dimTxt = oHraw > 0 ? `${oL.toFixed(2)}×${oHraw.toFixed(2)}` : oL.toFixed(2)
            this.doc.text(dimTxt + (isUebermessen ? ' ✓' : ''), ox, oy - 4, { width: oW, align: 'center' })
          }
        }

        // Dimension lines
        this.doc.save()
        this.doc.strokeColor('#555555').lineWidth(0.3)

        // Bottom: total width = 2(L+B)
        const dimYB = sy + skH + 6
        this.doc.moveTo(sx, sy + skH + 2).lineTo(sx, dimYB + 1.5).stroke()
        this.doc.moveTo(sx + skW, sy + skH + 2).lineTo(sx + skW, dimYB + 1.5).stroke()
        this.doc.moveTo(sx, dimYB).lineTo(sx + skW, dimYB).stroke()
        this.setFont('regular', 6)
        this.doc.fillColor('#444444')
        this.doc.text(`2×(${rL.toFixed(2)}+${rB.toFixed(2)}) = ${perim.toFixed(2)} m`, sx, dimYB + 2, { width: skW, align: 'center' })

        // Right: H
        const dimXR = sx + skW + 6
        this.doc.moveTo(sx + skW + 2, sy).lineTo(dimXR + 1.5, sy).stroke()
        this.doc.moveTo(sx + skW + 2, sy + skH).lineTo(dimXR + 1.5, sy + skH).stroke()
        this.doc.moveTo(dimXR, sy).lineTo(dimXR, sy + skH).stroke()
        // Rotated H label
        this.doc.save()
        this.doc.translate(dimXR + 4, sy + skH / 2)
        this.doc.rotate(-90)
        this.setFont('regular', 6)
        this.doc.fillColor('#444444')
        this.doc.text(`H: ${rH.toFixed(2)} m`, 0, 0, { width: skH, align: 'center' })
        this.doc.restore()

        this.doc.restore() // outer save

        y = sy + skH + 18
      }

      // ── DETAILS: Position table (right side or below sketch) ──
      let dy = sketchY
      const detX = hasSketch ? DET_X : 55
      const detValX = hasSketch ? DET_VAL_X : 200
      const detValW = hasSketch ? DET_VAL_W : 340

      // Positions
      for (const item of positions) {
        const unit = ['Wand', 'Bogen', 'Trap'].includes(item.unit) ? 'm²' : (item.unit || '')
        const result = item.result != null ? parseFloat(item.result).toFixed(2) : '—'
        this.setFont('regular', this.FONT_SIZES.tiny)
        this.doc.fillColor('#333333')
        this.doc.text(item.description || '—', detX, dy)
        this.doc.text(`${result} ${unit}`, detValX, dy, { width: detValW, align: 'right' })
        dy += 12
      }
      ...
      if (openings.length > 0) {
        dy += 2
        this.setFont('bold', this.FONT_SIZES.tiny)
        this.doc.fillColor('#c8503c')
        this.doc.text('Öffnungen:', detX, dy)
        dy += 11
        for (const op of openings) {
          ...
          this.setFont('regular', this.FONT_SIZES.tiny)
          this.doc.fillColor(isUebermessen ? '#22c55e' : '#c8503c')
          const label = (op.description || 'Öffnung') + (cnt > 1 ? ` ×${cnt}` : '')
          const valStr = isUebermessen
            ? `${totalArea.toFixed(2)} m² (überm.)`
            : `− ${totalArea.toFixed(2)} m²`
          this.doc.text(label, detX, dy)
          this.doc.text(valStr, detValX, dy, { width: detValW, align: 'right' })
          dy += 11
        }
      }
      ...
      const totalAbzug = openings.reduce((s, i) => {
        ...
        return singleArea >= vobThreshold ? s + total : s
      }, 0)
      const nettoM2 = Math.round((bruttoM2 - totalAbzug) * 100) / 100

      if (bruttoM2 > 0 && openings.length > 0) {
        dy += 3
        this.doc.save()
        this.doc.moveTo(detX, dy).lineTo(detValX + detValW, dy).strokeColor('#cccccc').lineWidth(0.5).stroke()
        this.doc.restore()
        dy += 5
        this.setFont('bold', this.FONT_SIZES.tiny)
        this.doc.fillColor('#333333')
        this.doc.text('Netto:', detX, dy)
        this.doc.text(`${nettoM2.toFixed(2)} m²`, detValX, dy, { width: detValW, align: 'right' })
        dy += 12
      }
  ...
  async addTotalsSection(invoiceData, majstorData) {
    ...
    console.log('Totals:', { subtotal, taxAmount, totalAmount, taxRate: effectiveVatRate + '%' })
  ...
  { align: 'right', width: 75 }
      ...
      this.doc.fillColor(this.COLORS.secondary)
      this.doc.text(kleinText, labelX, y, {
        width: 165,
        align: 'left',
        lineGap: 1.5
      })
  ...
  async generateInvoice(invoiceData, majstorData, aufmassDataList = []) {
    ...
    try {
      ...
      await this.addMalerAufmassRooms()
