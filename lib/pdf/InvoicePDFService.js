...
this.doc.text(
  ...
  async addAufmassPositions() {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      await this.checkSpaceAndAddPageIfNeeded(140)
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        for (let si = 0; si < segs.length; si++) {
          const sw = segWidths[si] * scale
          this.doc.moveTo(cx2, sketchY + sketchH + 1).lineTo(cx2, dimY + 2).stroke()
          this.doc.moveTo(cx2 + sw, sketchY + sketchH + 1).lineTo(cx2 + sw, dimY + 2).stroke()
          this.doc.moveTo(cx2, dimY).lineTo(cx2 + sw, dimY).stroke()
          this.doc.text(`${segWidths[si]}`, cx2 + 2, dimY + 3, { width: sw - 4, align: 'center' })
          cx2 += sw
        }
        // Total width
        if (segs.length > 1) {
          const totalDimY = dimY + 12
          this.doc.moveTo(sketchX, dimY + 4).lineTo(sketchX, totalDimY + 2).stroke()
          this.doc.moveTo(sketchX + sketchW, dimY + 4).lineTo(sketchX + sketchW, totalDimY + 2).stroke()
          this.doc.moveTo(sketchX, totalDimY).lineTo(sketchX + sketchW, totalDimY).stroke()
          this.doc.text(`${totalRealW}`, sketchX + 2, totalDimY + 3, { width: sketchW - 4, align: 'center' })
        }
        // Right: height of last segment
        const lastH = segHeights[segs.length - 1]
        const lastSH = lastH * scale
        let lastSY = sketchY
        if (align === 'bottom') lastSY = sketchY + sketchH - lastSH
        else if (align === 'center') lastSY = sketchY + (sketchH - lastSH) / 2
        const dimX = sketchX + sketchW + 6
        this.doc.moveTo(sketchX + sketchW + 1, lastSY).lineTo(dimX + 2, lastSY).stroke()
        this.doc.moveTo(sketchX + sketchW + 1, lastSY + lastSH).lineTo(dimX + 2, lastSY + lastSH).stroke()
        this.doc.moveTo(dimX, lastSY).lineTo(dimX, lastSY + lastSH).stroke()
        this.doc.save()
        this.doc.translate(dimX + 10, lastSY + lastSH)
        this.doc.rotate(-90)
        this.doc.text(`${lastH}`, 0, 0, { width: lastSH, align: 'center' })
        this.doc.restore()
        this.doc.restore()
        ...
        this.setFont('regular', 6)
        this.doc.fillColor('#999999')
        this.doc.text('Ansicht von innen', sketchX, sketchY + sketchH + (segs.length > 1 ? 28 : 14), { width: sketchW, align: 'center' })

        // Details
        const detX = sketchX + sketchW + 30
        const detValX = detX + 70
        this.setFont('regular', this.FONT_SIZES.small)
        this.doc.fillColor('#333333')
        let dy = sketchY
        ...
        for (const [label, value] of details) {
          this.setFont('bold', this.FONT_SIZES.small)
          this.doc.text(label, detX, dy, { continued: false })
          this.setFont('regular', this.FONT_SIZES.small)
          this.doc.text(value, detValX, dy)
          dy += 13
        }
      ...
      // Left: height
      if (totalPosH > 0) {
        const dimX = sketchX - 10
        this.doc.moveTo(sketchX - 1, sketchY).lineTo(dimX - 2, sketchY).stroke()
        this.doc.moveTo(sketchX - 1, sketchY + sketchH).lineTo(dimX - 2, sketchY + sketchH).stroke()
