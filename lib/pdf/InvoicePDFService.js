  ...
  async addAufmassPositions() {
    ...
    for (let pi = 0; pi < positions.length; pi++) {
      ...
      if (pos.preset === 'mehrteilig' && pos.segments?.length > 0) {
        ...
        // Details — position past all kote columns
        const dimRightEdge = sketchX + sketchW + 2 + (dimColsR - 1) * colSp + 12
        ...
        const detValW = 595.28 - 56.69 - detValX
        for (const [label, value] of details) {
          this.setFont('bold', this.FONT_SIZES.small)
          this.doc.text(label, detX, dy, { continued: false })
          this.setFont('regular', this.FONT_SIZES.small)
          const valH = this.doc.heightOfString(value, { width: detValW })
          this.doc.text(value, detValX, dy, { width: detValW })
      ...
      const dimRightEdgeStd = sketchX + sketchW + 2 + (heightDimCols - 1) * colSpH + 12
