...
export const customersAPI = {
  ...
  .select(`
    *,
    quotes_count:invoices!customer_email(count).eq.type.quote,
    invoices_count:invoices!customer_email(count).eq.type.invoice
  `)
  ...
  async autoSaveFromInvoice(invoiceData) {
    try {
       ...
       if (invoiceData.status === 'dummy') {
      console.log('Skipping customer creation for dummy invoice')
  ...
  .ilike('name', invoiceData.customer_name)
  .ilike('email', invoiceData.customer_email.toLowerCase().trim())
  .maybeSingle()
  ...
  async getStats(majstorId) {
    try {
      ...
      const { data: customers, error: customersError } = await supabase
        ...
        .eq('majstor_id', majstorId)
          .neq('name', 'DUMMY_ENTRY_FOR_NUMBERING') // SAMO DODAJ OVU LINIJU
  ...
  customer.name !== 'DUMMY_ENTRY_FOR_NUMBERING'
